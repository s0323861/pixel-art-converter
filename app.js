...
const els = Object.fromEntries(['dropZone','emptyState','canvasStage','fileInput','selectButton','outputCanvas','originalCanvas','pixelSize','sizeOutput','paletteGrid','ditherToggle','crtToggle','glitchToggle','downloadButton','videoButton','resetButton','status','compareControl','compareRange','compareLine'].map(id => [id, document.getElementById(id)]));
...
els.downloadButton.addEventListener('click',()=>{if(!sourceImage)return;els.outputCanvas.toBlob(blob=>{const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`${sourceName}-dot.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);els.status.textContent='PNGを保存しました';},'image/png');});
